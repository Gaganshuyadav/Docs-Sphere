...
import { ComponentStateType, setCurrentZoomNumber, setIsColorPaletteOpen, setIsHighlightColorPaletteOpen } from "../../../../redux/Slices/componentSlice";
...
export default function EditorToolbar() {
    ...
    const { isColorPaletteOpen, isHighlightColorPaletteOpen, currentZoomNumber } = useSelector((state: { component: ComponentStateType }) => state.component);
    ...
    return (
        <div className="flex bg-blue-50 rounded-full z-[5]">
            ...
            <div className=" h-10 w-full flex justify-start"  >
                ...
                {/* zoom option */}
                <Box sx={{ minWidth: 100, margin:"auto 10px" , height:"35px", display:{xs:"none", md:"flex"}}}>
                        <Select
                            value={ currentZoomNumber}
                            label="Age"
                            onChange={(e)=>{ dispatch( setCurrentZoomNumber(e.target.value))}}
                        >
                            {

                            [ 100, 120, 140, 160, 180, 200].map(( zoomNum, i)=>{
                                return <MenuItem key={i} value={zoomNum}>{zoomNum}%</MenuItem>
                            })
