...
import { emailNotVerified } from "./responses/index.js";
...
const userEmails = new Map<string ,Array<string>>();
    ...
    if(!token || !documentId){
        ...
        .then((isTokenActive)=>{
            ...
            if(!isTokenActive){
                ...
                jwt.verify(token , env.JWT_SECRET, ( err:VerifyErrors | null ,decoded: unknown)=>{
                    ...
                    if(err){
                        ...
                        (socket as any).user  = { id, email, roles};
        ...
        //current User Join
          socket.on( SocketEvent.CURRENT_USERS, ( data: { user: { id:number, email:string, roles: Array<string>, exp: number }, documentId:string } )=>{
          ...
          //current User Leave
          socket.on( SocketEvent.USER_DISCONNECT_FROM_CURRENT_USERS, ( data: { user: { id:number, email:string, roles: Array<string>, exp: number }, documentId:string } )=>{
          ...
          //Users disconnect
          socket.on("disconnecting", (data)=>{ //fully disconnected
            
            const documentId = socket.handshake.query.documentId as string |undefined;
            const user = socket.user as RequestUser | undefined;
          ...
          //Users disconnecting
          socket.on("disconnect", (data)=>{ //fully disconnected
